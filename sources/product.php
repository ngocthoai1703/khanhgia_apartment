...
{		
	...
	if($com=='san-pham-moi')
	{
		$where = " type='".$type."' and id_danhmuc=".$title_bar['id']." and hienthi=1 and spmoi=1 order by stt,id desc";	
	}
	if($com=='san-pham-ban-chay')
	{
		$where = " type='".$type."' and id_danhmuc=".$title_bar['id']." and hienthi=1 and spbanchay=1 order by stt,id desc";	
	}
	if($com=='san-pham-khuyen-mai')
	{
		$where = " type='".$type."' and id_danhmuc=".$title_bar['id']." and hienthi=1 and tieubieu=1 order by stt,id desc";	
	}
}
	//Tất cả sản phẩm
else
{
	...
	if($com=='san-pham-moi')
	{
		$where = " type='".$type."' and spmoi=1 and hienthi=1 order by stt,id desc";
		$com= "san-pham";
		$title_cat = "New Arrivals";
		$title = "New Arrivals";	
	}
	if($com=='san-pham-ban-chay')
	{
		$where = " type='".$type."' and spbanchay=1 and hienthi=1 order by stt,id desc";
		$com= "san-pham";
		$title_cat = "Best Sellers";
		$title = "Best Sellers";	
	}
	if($com=='san-pham-khuyen-mai')
	{
		$where = " type='".$type."' and tieubieu=1 and hienthi=1 order by stt,id desc";
		$com= "san-pham";
		$title_cat = "Sale";
		$title = "Sale";	
	}
...
if($id > 0)
{
	...
	$d->reset();
	$sql = "select id,ten$lang as ten,tenkhongdau,thumb,photo,masp,gia,giacu,mausac,size,spmoi,spbanchay,tieubieu FROM #_product where $where limit $bg,$pageSize";		
	$d->query($sql);

...
if(count($_SESSION['cart'])>0)
{
	...
	if(isset($_POST['hoten'])){	
		...
		$magiamgia = $_POST['magiamgiaan'];
		$d->reset();
		$sql = "select phantramgiamgia,solangiamgia from #_news_danhmuc where type='magiamgia' and ten='".$magiamgia."'";
		$d->query($sql);
		$solangiamgia = $d->fetch_array();
		$phantramgiamgia = $solangiamgia['phantramgiamgia'];
		//validate dữ liệu
		$httt = (int)$httt;
		...
		{	
			...
			//dump($tonggia);
			$sql = "INSERT INTO  table_donhang (httt,madonhang,hoten,dienthoai,thanhpho,quan,diachi,email,tonggia,noidung,ngaytao,tinhtrang,ngaycapnhat,ip,id_user,phiship,phantramgiamgia) 
			VALUES ('$httt','$madonhang','$hoten','$dienthoai','$thanhpho','$quan','$diachi','$email','$tonggia','$noidung','$ngaydangky','1','$ngaycapnhat','$ip','$id_user','$phiship','$phantramgiamgia')";	
		...
		#Nếu insert bảng đơn hàng thành công thì tiếp tự insert vào bảng chi tiết đơn hàng
			...
			{
				$d->reset();
				$slgiamgia = $solangiamgia['solangiamgia'] - 1;
				$sqlgiamgia = "Update table_news_danhmuc set solangiamgia='".$slgiamgia."' where ten='".$magiamgia."' ";
				$d->query($sqlgiamgia);
				...
				#Nếu insert bảng chitietdonhang thành công thì bắt đầu gửi mail
					...
					*=====================================*/
					...
					<tr>
					<th colspan="2"><a href="'.$_SERVER["SERVER_NAME"].'">LESU Store</a></th>
					</tr>
					<tr>
					<th colspan="2">&nbsp;</th>
					</tr>
					<tr>
					<th style="text-align:left">Xin chào '.$hoten.',<br>
					Cám ơn bạn đã đặt hàng tại LESU Store.<br>
					Hi vọng bạn có một trải nghiệm mua sắm thú vị tại LESU Store<br>
					LESU Store đang xử lý đơn hàng của bạn và một kiện hàng thuộc đơn hàng #'.$madonhang.' đã được đóng gói và giao cho đơn vị vận chuyển. Vui lòng xem nội dung bên dưới để biết thêm chi tiết.<br>
					Nếu bạn có thắc mắc về đơn hàng của mình, xin vui lòng liên hệ LESU Store qua email: '.$company['email'].'<br>
					Thân ái,<br>
					LESU Store</th>
					</tr>
					<tr>
					...
					<tr>
					<th>Mã đơn hàng :'.$madonhang.'</th>
					</tr>
					<tr>
					<th>Họ tên : '.$hoten.'</th>
					</tr>
					<tr>
					<th>Địa chỉ : '.$diachi.'</th>
					</tr>
					<tr>
					<th>Email : '.$email.'</th>
					</tr>
					<tr>
					<th>Điện thoại : '.$dienthoai.'</th>
					</tr>

					<tr>
					<th>Nội dung : '.$noidung.'</th>
					</tr>
					';
					$body .= '</table>';
					...
					if(is_array($_SESSION['cart']))
					{
						$body.= '<tr bgcolor="#F0F0F0" height="55px"><td align="center">STT</td><td style="text-align:center;">Hình S.Phẩm</td><td style="text-align:center;" class="gh_an">Tên sản phẩm</td><td align="center">Size / Màu</td><td align="center">Đơn giá</td><td align="center">Số lượng</td> <td align="center">Thành tiền</td></tr>';
						$max=count($_SESSION['cart']);
						for($i=0;$i<$max;$i++){
							$pid=$_SESSION['cart'][$i]['productid'];
							$size=$_SESSION['cart'][$i]['size'];
							...
							$body.='<td width="25%" style="padding:0px 10px; box-sizing:border-box;">'.$size.' - <span style="width:20px;height:20px;display:inline-block;border:1px solid #000;background-color:'.$mausac.'"></span</td>';			
						...
						<td colspan="6" style="background:#F0F0F0; height:55px; text-align:right; padding-right:10px;">Tổng tiền</td>
						...
						<td colspan="6" style="background:#F0F0F0; height:55px; text-align:right; padding-right:10px;">Phí ship</td>

						<td style="background: #fff;text-align: center;">'.number_format($phiship,0, ',', '.').'&nbsp;<sup>đ</sup></td>

						</tr>

						<tr>

						<td colspan="6" style="background:#F0F0F0; height:55px; text-align:right; padding-right:10px;">Tổng thanh toán</td>

						<td style="background: #fff;text-align: center;">'.number_format(get_order_total()*((100-$phantramgiamgia)/100)+$phiship,0, ',', '.').'&nbsp;<sup>đ</sup></td>
				   ...
				   #------------Chi tiết đơn hàng---------------------
					...
					$_SESSION['cart']=0;
					unset($_SESSION['cart']);
					$tb=_dathangthanhcong;
					if($mail->Send())
					{

						transfer($tb.$madonhang, "http://".$config_url);
					}
					else

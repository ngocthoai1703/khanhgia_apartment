...
<form name="supplier" id="validate" class="form" action="index.php?com=news&act=save&p=<?=$_REQUEST['p']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
     ...
     <div class="widget">
      ...
      <?php if($_GET['type']=='duan' or $_GET['type']=='congtrinh' or $_GET['type']=='album' or $_GET['type']=='thuvien' or $_GET['type']=='stories'or $_GET['type']=='img_our_store' ) { ?>  
      <div class="formRow">
    ...
    <?php }?>
 	...
 	<div class="formRow">
       ...
       <div id="content_lang_<?=$key?>" class="tab_content">        
        ...
        <?php if($_GET['type']=='stories'){ ?>
            <div class="formRow">
            <label>Mô tả ngắn: <img src="./images/question-button.png" alt="Chọn loại"  class="icon_que tipS" original-title="Viết mô tả ngắn"> </label>
            <div class="formRight"><textarea class="ck_editor" name="mota<?=$key?>" id="mota<?=$key?>" rows="8" cols="60"><?=@$item['mota'.$key]?></textarea>
            </div>
            <div class="clear"></div>
        </div>
        <?php } else { ?>

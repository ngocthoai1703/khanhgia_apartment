...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_tintuc,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 280, 220, _upload_tintuc,$file_name,1);									
			$d->setTable('news');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
	             ...
	             for($i=0;$i<count($_FILES['files']['name']);$i++){
	            	if($_FILES['files']['name'][$i]!=''){
						if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
						{
							...
							$data1['photo'] = $photo;
							$data1['thumb'] = create_thumb($data1['photo'], 280, 220, _upload_hinhthem,$file_name,1);	
...
function save_danhmuc(){
	...
	if($id){		
		...
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
		$data['ngaysua'] = time();
		$data['phantramgiamgia'] = $_POST['phantramgiamgia'];
		$data['solangiamgia'] = (int)$_POST['solangiamgia'];
		$data['title'] = $_POST['title'];
		$data['keywords'] = $_POST['keywords'];
			$data['phiship'] = $_POST['phiship'];
		$data['description'] = $_POST['description'];
		foreach ($config['lang'] as $key => $value) {
			$data['ten'.$key] = $_POST['ten'.$key];
			$data['mota'.$key] = magic_quote($_POST['mota'.$key]);

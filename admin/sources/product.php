...
function get_items(){
	...
	if($_REQUEST['key']!='')
	{
		$where.=" and ten like '%".$_REQUEST['key']."%' or masp like '%".$_REQUEST['key']."%'";
	}
...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			...
			$data['photo'] = $photo;	
			//dump($photo);
			$data['thumb'] = create_thumb($data['photo'], 350 , 350, _upload_sanpham,$file_name,1);									
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
	            ...
	            for($i=0;$i<count($_FILES['files']['name']);$i++){
	            	if($_FILES['files']['name'][$i]!=''){
						if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
						{
							...
							$data1['photo'] = $photo;
							$data1['thumb'] = create_thumb($data1['photo'], 350 , 350, _upload_hinhthem,$file_name,1);	
...
function save_danhmuc(){
	...
	if($id){		
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 678, 730, _upload_sanpham,$file_name,1);

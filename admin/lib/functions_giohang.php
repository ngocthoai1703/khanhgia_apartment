...
function get_product_tenkhongdau($pid){
	global $d, $row;
...
function addtocart($pid,$size,$mausac,$q)
{
	...
	{
		if(product_exists($pid,$size,$mausac))
		{		
				//New		
			for ($i=0; $i < count($_SESSION['cart']); $i++) 
			{ 
				if($_SESSION['cart'][$i]['productid']==$pid && $_SESSION['cart'][$i]['size']==$size &&  $_SESSION['cart'][$i]['mausac']==$mausac)
				{
